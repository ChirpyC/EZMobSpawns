...
public class SpawnPropertyGroup {
        ...
        for(Placement p : validPlacements){
            switch (p){
                case SURFACE -> {
                    ...
                    BlockPos ceil = pos.atY(surfaceY-1);
                    isValid = (world.canSeeSky(pos) || world.getBlockState(ceil).is(BlockTags.LEAVES))
                            && world.getBlockState(pos).isAir() && world.getBlockState(pos.below()).isSolid();
                }
                case IN_WATER -> {
                    int surfaceY = (int) (world.getHeight(Heightmap.Types.WORLD_SURFACE, pos.getX(), pos.getZ()));
                    BlockPos ceil = pos;
                ...
                case CAVE_WATER -> {
                    int surfaceY = (int) (world.getHeight(Heightmap.Types.WORLD_SURFACE, pos.getX(), pos.getZ()) - 1.0f);
                    BlockPos ceil = pos;
                    while(ceil.getY() < surfaceY && !world.getBlockState(ceil.above()).isSolid()){
                        ceil = ceil.above();
                    }
                ...
                case IN_LAVA -> {
                    isValid = !world.getBlockState(pos).isSolid() && world.getBlockState(pos.below()).is(Blocks.LAVA);
                }
                case AIR -> {
    ...
    public static boolean isValidTemperature(ArrayList<Range<Float>> validTemperature, ServerLevel world, BlockPos pos) {
        return shouldSkipCheck(validTemperature) || validTemperature.stream().anyMatch(temp -> temp.isWithinRange(world.getBiome(pos).value().getBaseTemperature()));
    }
    ...
    public static boolean isValidWeather(ArrayList<Biome.Precipitation> validWeather, boolean activateIfStormy, ServerLevel world, BlockPos pos) {
        return (activateIfStormy && world.isThundering()) ||
                ...
                validWeather.stream().anyMatch(weather -> {
                    switch (weather){
                        case SNOW -> {
                            return world.getBiome(pos).value().hasPrecipitation() && world.isRaining() && world.getBiome(pos).value().coldEnoughToSnow(pos);
                        }
                        case RAIN -> {
                            return world.getBiome(pos).value().hasPrecipitation() && world.isRaining() && world.getBiome(pos).value().warmEnoughToRain(pos);
                        }
                        case NONE -> {
                            return world.getBiome(pos).value().hasPrecipitation() || !world.isRaining();
                        }
                    }
                    return false;
                });

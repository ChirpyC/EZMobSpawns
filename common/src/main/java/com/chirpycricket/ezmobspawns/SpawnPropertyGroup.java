    ...
    public static boolean isValidWeather(ArrayList<Biome.Precipitation> validWeather, boolean activateIfStormy, ServerLevel world, BlockPos pos) {
        return (activateIfStormy && world.isThundering()) ||
                ...
                validWeather.stream().anyMatch(weather -> {
                    switch (weather){
                        case SNOW -> {
                            return world.getBiome(pos).getPrecipitation().equals(Biome.Precipitation.SNOW) && world.getBiome(pos).isColdEnoughToSnow(pos);
                        }
                        case RAIN -> {
                            return world.getBiome(pos).getPrecipitation().equals(Biome.Precipitation.RAIN) && world.isRaining() && !world.getBiome(pos).isColdEnoughToSnow(pos);
                        }
                        case NONE -> {

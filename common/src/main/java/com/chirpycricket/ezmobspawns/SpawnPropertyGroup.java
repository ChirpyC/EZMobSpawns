    ...
    public static boolean isValidWeather(ArrayList<Biome.Precipitation> validWeather, boolean activateIfStormy, ServerLevel world, BlockPos pos) {
        return (activateIfStormy && world.isThundering()) ||
                ...
                validWeather.stream().anyMatch(weather -> {
                    switch (weather){
                        case SNOW -> {
                            return world.getBiome(pos).value().getPrecipitation().equals(Biome.Precipitation.SNOW) && world.getBiome(pos).value().coldEnoughToSnow(pos);
                        }
                        case RAIN -> {
                            return world.getBiome(pos).value().getPrecipitation().equals(Biome.Precipitation.RAIN) && world.isRaining() && world.getBiome(pos).value().warmEnoughToRain(pos);
                        }
                        case NONE -> {
                            return world.getBiome(pos).value().getPrecipitation().equals(Biome.Precipitation.NONE) || !world.isRaining();
                        }
                    }
                    return false;
                });

...
public class PlacementHelper {
    ...
    public static BlockPos findClearSpaceNear(ServerLevel world, BlockPos pos, EntityType type, boolean airAllowed){
        ...
        if(!airAllowed) {
            ...
            } while(world.noBlockCollision(null, bb.move(0,-1,0)) && i < 20);
        }

        if(world.noBlockCollision(null, bb)) return pos;
        else return null;
    }
    ...
    public static BlockPos getLowestSurface(ServerLevel world, BlockPos p1, EntityType type){
        ...
        while( !world.getBlockState(pos.below()).liquid()
                && ((!world.getBlockState(pos.below()).isSolid()
                        && world.noBlockCollision(null, bb.move(0,-1,0)))
                    || (world.getBlockState(pos.below()).isSolid()

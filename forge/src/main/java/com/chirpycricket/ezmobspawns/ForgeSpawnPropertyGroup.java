...
public class ForgeSpawnPropertyGroup {
    ...
    ArrayList<Biome.RainType> weather= new ArrayList();
    ...
    public boolean coarseCheck(ServerWorld world, BlockPos pos){
    ...
    public boolean fineCheck(ServerWorld world, BlockPos pos, MobSpawnProperties properties){
        boolean isValid =
    ...
    public static boolean checkMobLimit(MobSpawnProperties properties, ServerWorld world, BlockPos pos, int totalNewMobs){
        if(properties == null || !properties.shouldLimit()) return true;
        EntityType type = getEntityType(properties.entityType);
        if(type == null) return false;

        List<? extends Entity> nearbyEntities = world.getEntitiesOfClass(type.create(world).getClass(), new AxisAlignedBB(pos).inflate(128));
    ...
    public static boolean hasRequiredBlocks(ArrayList<String> allowedBlocks, ServerWorld world, BlockPos pos){
    ...
    public static boolean isValidGroundBlock(ArrayList<String> allowedBlocks, ServerWorld world, BlockPos pos){
    ...
    public static boolean isNotInvalidGroundBlock(ArrayList<String> disallowedBlocks, ServerWorld world, BlockPos pos){
    ...
    public static boolean isValidPlacement(ArrayList<Placement> validPlacements, ServerWorld world, BlockPos pos){
        if (shouldSkipCheck(validPlacements)) return true;
        boolean isValid = false;
        for(Placement p : validPlacements){
            int surfaceY = (int) (world.getHeight(Heightmap.Type.WORLD_SURFACE, pos.getX(), pos.getZ()));
            ...
            switch (p){
                ...
                case IN_WATER:
                    ...
                    break;
                case CAVE_WATER:
                    surfaceY = (int) (world.getHeight(Heightmap.Type.WORLD_SURFACE, pos.getX(), pos.getZ()) - 1.0f);
                    ceil = pos;
    ...
    public static boolean isValidLightLevel(ArrayList<Range<Integer>> validLightLevels, ServerWorld world, BlockPos pos){
    ...
    public static boolean isValidTemperature(ArrayList<Range<Float>> validTemperature, ServerWorld world, BlockPos pos) {
    ...
    public static boolean isValidWeather(ArrayList<Biome.RainType> validWeather, boolean activateIfStormy, ServerWorld world, BlockPos pos) {
        return (activateIfStormy && world.isThundering()) ||
                ...
                validWeather.stream().anyMatch(weather -> {
                    switch (weather){
                        case SNOW:
                            return world.getBiome(pos).getPrecipitation().equals(Biome.RainType.SNOW) && world.getBiome(pos).getTemperature(pos) < .15f;
                        case RAIN:
                            return world.getBiome(pos).getPrecipitation().equals(Biome.RainType.RAIN) && world.isRaining() && world.getBiome(pos).getTemperature(pos) >= .15f;
                        case NONE:
                            return world.getBiome(pos).getPrecipitation().equals(Biome.RainType.NONE) || !world.isRaining();
                    }
                    return false;
                });
    }
    public static boolean isValidMoonphase(ArrayList<MoonPhases> validMoonPhases, ServerWorld world) {
        return shouldSkipCheck(validMoonPhases) || validMoonPhases.stream().anyMatch(phase -> phase == MoonPhases.values()[world.getMoonPhase()]);
    }

    public static boolean isValidTime(ArrayList<Range<Integer>> validTimeframes, ServerWorld world) {
    ...
    public static boolean isValidDimension(ArrayList<String> validDimensions, ServerWorld world) {
        return shouldSkipCheck(validDimensions) || validDimensions.contains(world.dimension().location().toString());
    }

    public static boolean isValidBiome(ArrayList<String> validBiomes, ServerWorld world, BlockPos pos){
...
//                })
    ...
    public EffectInstance getEffect(){
        ...
        Effect effect = getEffectByString(effectProperties.effect);
        if(effect == null) return null;
        return new EffectInstance(effect, effectProperties.duration);
    }
    ...
    public void printCoarseCheck(ServerWorld world, BlockPos pos, boolean failuresOnly){
    ...
    public void printFineCheck(ServerWorld world, BlockPos pos, boolean failuresOnly){
